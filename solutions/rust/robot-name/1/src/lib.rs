...
impl Robot {
    ...
    pub fn reset<R: Rng>(&mut self, rng: &mut R) {
        ...
        let name_used_set_rc = self.name_used_set.clone();
        name_used_set_rc.borrow_mut().remove(self.name());

        if name_used_set_rc.borrow().contains(&new_name) {
            self.reset(rng);
        } else {
            let mut name_used_set = name_used_set_rc.borrow_mut();
