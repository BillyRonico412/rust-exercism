...
impl BowlingGame {
    ...
    fn handle_basic_game(&mut self, pins: u16) -> Result<(), Error> {
        match self.frame_rolls {
            (None, None) if pins == 10 => Ok(self.update_frame(Frame::Strike)),
            (None, None) => Ok(self.frame_rolls.0 = Some(pins)),
            (Some(x), None) => {
                if x + pins > 10 {
                    Err(Error::NotEnoughPinsLeft)
                } else if x + pins == 10 {
                    Ok(self.update_frame(Frame::Spare(x)))
                } else {
                    Ok(self.update_frame(Frame::OpenFrame(x, pins)))
                }
            }
        ...
        match self.frame_rolls {
            ...
            (Some(x), None) => {
                if x < 10 && x + pins > 10 {
                    ...
                    Ok(self.update_frame(Frame::TenthFrame(x, pins, 0)))
                } else {
                    Ok(self.frame_rolls.1 = Some(pins))
                }
            }
            (Some(x), Some(y)) => {
                if x == 10 && y != 10 && y + pins > 10 {
                    ...
                    Ok(self.update_frame(Frame::TenthFrame(x, y, pins)))
                }
            }
            _ => Err(Error::NotEnoughPinsLeft),
        }
